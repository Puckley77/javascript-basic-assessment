...
var number = heroes.indexOf('batman');
var thirdHero = heroes[number];
...
function doubleTheFun(param){
    ...
    if(typeof(param) === 'number'){
        return param * 2;
    }else if(hasNumber.test(param)){
        param = Number(param) * 2;
        return param;
    }else if(typeof(param) === 'string'){
        param = param.repeat(2);
        return param;
    }
}
...
function getValueOfProperty(obj, thing){
	return obj[thing];
}
...
function makeChatMessage(msg, athr){
	var chat = {
		message: msg,
		author: athr,
		timestamp: new Date()
